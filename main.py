...
async def crawl(request: Request):
    ...
    try:
        result,external_urls = await crawler.run_crawler(url, max_urls, respect_robot_flag)  

        #print(f"&&&&External URLs: {external_urls}")

        return {"message": "Crawl completed", "scraped_urls": result, "external_urls": external_urls}
